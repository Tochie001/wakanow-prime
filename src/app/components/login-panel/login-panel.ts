...
/**
 * The login card content (Google / Apple / email + demo accounts), shared by the
 * /login page and the tier-selection popup. Performs the demo login and emits
 * `loggedIn`; emits `guest` when the optional "continue as guest" path is taken.
 */
@Component({
  ...
  template: `
<div class="lp">
  <button type="button" class="lp-social" (click)="social()">
  ...
  <button type="button" class="lp-continue" (click)="social()">Continue</button>

  <p class="lp-signup">Don't have an account?<a href="#" (click)="$event.preventDefault()">Sign up</a></p>

  <div class="lp-divider lp-divider-demo"><span>or use a demo account</span></div>

  <div class="lp-demo">
    <button type="button" class="lp-demo-btn lp-demo-prime" (click)="enterPrime()">
      <span class="lp-demo-icon" aria-hidden="true"><app-prime-mark /></span>
      <span class="lp-demo-text">
        <strong>Log in as a Prime member</strong>
        <small>Gbolahan Shobande · Family plan</small>
      </span>
    </button>
    <button type="button" class="lp-demo-btn" (click)="enterNonMember()">
      <span class="lp-demo-icon lp-demo-icon-plain" aria-hidden="true">○</span>
      <span class="lp-demo-text">
  ...
  @if (showGuest()) {
    ...
    <button type="button" class="lp-guest" (click)="continueGuest()">Continue as guest</button>
  }
</div>
  `,
})
export class LoginPanel {
  ...
  showGuest = input(false);
  loggedIn = output<void>();
  guest = output<void>();

  /** Google / Apple / email all log in as the demo account in this prototype. */
  social() {
